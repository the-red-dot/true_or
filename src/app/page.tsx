...
export default function TruthOrDareGame() {
  ...
  // --- Auto-adjust heat level based on active player's limits ---
  useEffect(() => {
    if (lastActivePlayer && lastActivePlayer.max_heat_level) {
        if (heatLevel > lastActivePlayer.max_heat_level) {
            setHeatLevel(lastActivePlayer.max_heat_level);
        }
    }
  }, [lastActivePlayer, heatLevel, setHeatLevel]);
          ...
          <div className="flex flex-col items-center w-full max-w-6xl h-full justify-center">
            ...
            <div className="mt-12 bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10 flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="flex gap-4 items-center bg-black/30 p-2 rounded-2xl">
                  {[1, 2, 3].map((level) => {
                      const maxAllowedHeat = lastActivePlayer?.max_heat_level ?? 3;
                      const isDisabled = level > maxAllowedHeat;

                      return (
                        <button
                          key={level}
                          onClick={() => {
                              if (!isDisabled) setHeatLevel(level);
                          }}
                          disabled={isDisabled}
                          className={`
