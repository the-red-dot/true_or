...
import { Flame, Trash2, LogOut, User as UserIcon, WifiOff, RefreshCw, Cpu, Beer, ThumbsUp, ThumbsDown, LogIn, Play } from "lucide-react";
...
export default function TruthOrDareGame() {
  ...
  const stateRef = useRef({ players, lastActivePlayer, gameState, authUser });

  useEffect(() => {
    stateRef.current = { players, lastActivePlayer, gameState, authUser };
  }, [players, lastActivePlayer, gameState, authUser]);
  ...
  useEffect(() => {
    ...
    const setupRealtime = async (userId: string) => {
      ...
      channel = supabase.channel(`room_${userId}`)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'players', filter: `host_id=eq.${userId}` }, (payload) => {
          setPlayers(prev => [...prev, payload.new as Player]);
        })
        .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'players', filter: `host_id=eq.${userId}` }, (payload) => {
          // עדכון רשימת השחקנים
          setPlayers(prev => {
          ...
          // גישה דרך ה-Ref כי ה-Closure של הפונקציה הזו נוצר רק פעם אחת
          if (stateRef.current.gameState !== 'lobby' && stateRef.current.gameState !== 'waiting_for_spin') {
             // שים לב: אנחנו לא יודעים מי ה-selectedPlayer בתוך הקלוז'ר הישן, לכן לא בודקים אותו כאן ישירות
             // אלא סומכים על ה-useEffect של ניהול תורות שיטפל בזה אם השחקן נעלם מהמערך
          }
        })
    ...
    const handleGameEvent = async (data: any) => {
      ...
      // --- טיפול קריטי: שחקן עזב דרך הטלפון ---
      if (type === 'player_left') {
        // 1. הסרה ויזואלית מיידית
        setPlayers(prev => prev.filter(p => p.id !== playerId));
        
        // 2. מחיקה מהדאטה בייס (הוסט מבצע את הפעולה כי לשחקן אין הרשאה)
        if (stateRef.current.authUser) {
            console.log("Removing player from DB:", playerId);
            await supabase.from('players').delete().eq('id', playerId).eq('host_id', stateRef.current.authUser.id);
        }
      }
    ...
    return () => {
      ...
      if (channel) supabase.removeChannel(channel);
      // מחיקה ביציאה היא מסוכנת ב-Hot Reload, אבל ביציאה אמיתית זה טוב
      // if (authUser) { supabase.from('game_states').delete().eq('host_id', authUser.id); }
    };
  }, []);
  ...
  const resetGame = async (askConfirm = true) => {
    ...
    if (playersError) {
      ...
      alert("שגיאה במחיקת שחקנים מהמסד נתונים. נסה שוב.");
      return;
    }
    ...
    // 2. איפוס הסטייט ב-DB כדי שהטלפונים יקלטו שהמשחק נגמר/אופס
    await supabase.from('game_states').upsert({
      host_id: authUser.id,
      status: 'lobby',
    ...
    // 3. רק בסוף עדכון המסך המקומי (כדי למנוע מצב שזה נעלם וחוזר)
    setPlayers([]);
    setGameState('lobby');
  ...
  return (
    <main className="h-screen w-full bg-black text-white font-sans overflow-hidden relative selection:bg-pink-500 flex flex-col" dir="rtl">
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative z-10 p-10 h-full">
        ...
        <AnimatePresence>
          {gameState === 'penalty' && (
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.2 }} className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-red-900/90 backdrop-blur-md overflow-hidden" >
        ...
        {authUser && joinUrl && (
          <div className={`absolute z-30 transition-all duration-500 bg-white p-2 rounded-xl shadow-2xl ${
            gameState === 'lobby' || gameState === 'waiting_for_spin' ? 'bottom-20 right-10 scale-125 rotate-3 hover:rotate-0' : 'bottom-6 right-6 scale-75 opacity-70 hover:opacity-100'
          }`}>
